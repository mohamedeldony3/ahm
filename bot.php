	...
	$callback = function ($update, $bot) {
      ...
      $accounts = json_decode(file_get_contents('accounts.json'),1);
		if(isset($update->message)){
				...
				$token = $config['token'];
				$Spe = file_get_contents("https://api.telegram.org/bot/getChatMember?chat_id=@AEETCHI&user_id=$chatId");
				$Spe2 = file_get_contents("https://api.telegram.org/bot/getChatMember?chat_id=@akil828&user_id=$chatId");
				$url_info = file_get_contents("https://api.telegram.org/bot$token/getMe");
				$json_info = json_decode($url_info);
				$userr = $json_info->result->username;
				$ban = explode("\n",file_get_contents("ban.txt"));
				$cod = explode("\n",file_get_contents("cod.txt"));
				$codd = explode("\n",file_get_contents("codd.txt"));
			...
			if(strpos($text,"/zz")!==false){
	$olo = explode("\n",file_get_contents("cod.txt"));
			
    $rur = explode('/zz ',$text)[1];

       if(in_array($rur, $olo) and !in_array($rur, $codd) ){     
       
       if(!in_array($chatId, $mem)){
       
         file_put_contents("mem.txt", $chatId."\n",FILE_APPEND);
         file_put_contents("codd.txt", $rur."\n",FILE_APPEND);
       $bot->sendMessage([
'chat_id'=>$chatId,
...
الكود : $rur ",
		...
		$bot->sendMessage([
'chat_id'=>$chatId,
'text'=>"انت مسجل من قبل خلي فرصه لغيرك",
		]);
		}
  }elseif(in_array($rur, $olo) and in_array($rur, $codd)){
...
'text'=>"تم حظرك ",
              ...
              $bot->sendphoto([ 'chat_id'=>$chatId,
                  'photo'=>"https://t.me/Ahmed_Salah_2",
                   'caption'=>'𓊈 𝑊𝐸𝐿𝐿𝐶𝑀𝐸 𓊉 𓁹 التفعيل بواسطة @B_3_G',
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
          	...
          	if($config['mode'] != null){
                  ...
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
                          ...
                          [['text'=>' قناتي❗','url'=>'t.me/Ahmed_Salah_2'],['text'=>'المطور❔','url'=>'t.me/B_3_G']],
                      ]
                  ])
                  ]);
          		    $config['mode'] = null;
	          ...
	          if($text == "/start" and $chatId != $id and !in_array($chatId, $ban)) {
	          mkdir("$chatId");
				$akilid = explode("\n",file_get_contents("ids.txt"));
				if(!in_array($chatId, $mem)){
                file_put_contents("ids.txt", $chatId."\n",FILE_APPEND);
                					...
                					'text'=>"لا يمكنك استخدام البوت الا بكود من الادمن ارسل الكود هكذا  /zz xx  استبدل xx بل كود",
	 
              					
                			       
                			                     			       ]);
                }elseif($text == "/start" and $chatId != $id and !in_array($chatId, $ban)  and in_array($chatId, $mem)){
                					...
                					'text'=>"اهلا في بوت مصنع  777بوتات الصيد",
        		...
        		$keyboard = ['inline_keyboard'=>[
                 ...
                 'chat_id'=>$chatId,
                  ...
                  'reply_markup'=>json_encode([
		              ...
		              } elseif($data == 'dletr' and in_array($chatId, $mem)){
                ...
                $bot->answerCallbackQuery([
                 ...
                 'chat_id'=>$chatId,
                  'text'=>"https://t.me/akil828/3189",
		              ...
		              } elseif($data == 'maker' and !in_array($chatId,$ban) and in_array($chatId, $mem)){
		              mkdir("$chatId");
     ...
     $bot->sendMessage([
                      ...
                      'text'=>"معلومات البوت 
                      بوت مدفوع
                      لشراء ملف البوت تواصل معاء المطور@B_3_G 
                                                  معرفي : @B_3_G",   
                  'reply_markup'=>json_encode([
                      'inline_keyboard'=>[
		...
		} elseif($data == 'maker1' and !in_array($chatId, $ban) and in_array($chatId, $mem)){
